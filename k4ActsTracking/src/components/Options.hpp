...
namespace ActsExamples {
  namespace Options {
    ...
    template <size_t kSize> class Reals : public std::array<double, kSize> {};
    ...
    /// @note Implemented as a subclass so it is distinct from `std::array`
    ///   and we can provide overloads in the same namespace.
    template <size_t kSize> class Integers : public std::array<int, kSize> {};
    ...
    ///   undistinguishable.
    template <size_t kSize> inline std::istream& operator>>(std::istream& is, Reals<kSize>& values) {
    ...
    template <size_t kSize> inline std::ostream& operator<<(std::ostream& os, const Reals<kSize>& values) {
    ...
    ///   and additional command line both start with `-` and would be
    ///   undistinguishable.
    template <size_t kSize> inline std::istream& operator>>(std::istream& is, Integers<kSize>& values) {
    ...
    template <size_t kSize> inline std::ostream& operator<<(std::ostream& os, const Integers<kSize>& values) {
      detail::printIntegers(os, kSize, values.data());
      return os;
    }

    /// Print a variable number of integers as `x:y:z:...`.
    inline std::ostream& operator<<(std::ostream& os, const VariableIntegers& values) {
