...
namespace ActsExamples {
  ...
  public:
    ...
    template <typename T> void add(const std::string& name, T&& object);
...
inline ActsExamples::WhiteBoard::WhiteBoard(std::unique_ptr<const Acts::Logger> logger) : m_logger(std::move(logger)) {}

template <typename T> inline void ActsExamples::WhiteBoard::add(const std::string& name, T&& object) {
  if (name.empty()) {
...
template <typename T> inline const T& ActsExamples::WhiteBoard::get(const std::string& name) const {
