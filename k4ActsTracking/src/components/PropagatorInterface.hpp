...
namespace ActsExamples {
  ...
  public:
    ...
    virtual PropagationOutput execute(const AlgorithmContext& context, const PropagatorAlg::Config& cfg,
  ...
  /// Setting up a @c PropagatorAlg with this looks like:
  /// ```cpp
  ...
  template <typename propagator_t> class ConcretePropagator : public PropagatorInterface {
  public:
    ConcretePropagator(propagator_t propagator) : m_propagator{std::move(propagator)} {}

    PropagationOutput execute(const AlgorithmContext& context, const PropagatorAlg::Config& cfg,
                              Acts::LoggerWrapper               logger,
  ...
  private:
    ...
    PropagationOutput executeTest(const AlgorithmContext& context, const PropagatorAlg::Config& cfg,
      ...
      if (cfg.mode == 0) {
        ...
        PropagatorOptions options(context.geoContext, context.magFieldContext, Acts::LoggerWrapper{logger()});
